...
def get_steps(num_iters = num_iters, l = l, T=T ,  n_max = n_max, n_min = n_min,  jumps = jumps, q=q, seeds=[0,1,2,3]):
...
def sim_2_ss_bb(l=10, nodes=[6, 6, 6, 6], estimates = [4, 2, 3, 4]):
    # simulate 2-SS-BB with trial of length l blocks, taking (T-1)l slots to execute
    T = len(nodes)
    ...
    bit_pattern = gen_pattern_2_ss_bb(num_nodes_in_blocks, estimates)
    return num_nodes_in_blocks, bit_pattern

def gen_pattern_2_ss_bb(num_nodes_in_blocks, estimates):
    # given number of nodes in blocks and previous estimates, generate a tx pattern according to 2-SS-BB
    ...
    bit_pattern = np.array([[symbol('0') for b in range(T//2)] for block in range(l)])
    no_of_nodes = T
    # 2SS
    ...
    for block in range(l):
        for b in range(T):
            ...
            for i in range(num_nodes_b_block):
                if(no_of_nodes%2 == 0 and b <= (no_of_nodes//2)-1):#T is even && T<=T//2 4,2;6,3;8,4
                    bit_pattern[block]+=M[b]
                elif(no_of_nodes%2 == 0 and b >= (no_of_nodes//2)+1):#T is even && T>=T
                    bit_pattern[block]+=M[b]
...
def train_student_offline(teacher,tag, alpha=alpha, test_train_split=0.9, epochs=200, batch_size=16):
    ctag = f"train_student_{tag}_l{int(l)}_T{T}_j{jumps}_n{num_iters}"
    fname = f"./data/{ctag}.csv"
